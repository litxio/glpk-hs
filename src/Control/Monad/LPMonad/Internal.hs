...
runLPM :: (Ord v, Group c, AbelianAdditive c)
       => LPM v c a -> (a, LP v c)
runLPM = runIdentity . runLPT

runLPT :: (Ord v, Group c, AbelianAdditive c)
       => LPT v c m a -> m (a, LP v c)
...
execLPT :: (Ord v, Group c, Monad m, AbelianAdditive c)
        => LPT v c m a -> m (LP v c)
...
evalLPT :: (Ord v, Group c, Monad m, AbelianAdditive c) => LPT v c m a -> m a
...
{-# SPECIALIZE equal :: (Ord v, Group c, AbelianAdditive c) => LinFunc v c -> LinFunc v c -> LPM v c (),
        (Ord v, Group c, Monad m, AbelianAdditive c) => LinFunc v c -> LinFunc v c -> LPT v c m () #-}
...
equal, leq, geq :: (Ord v, Group c, MonadState (LP v c) m, AbelianAdditive c)
                => LinFunc v c -> LinFunc v c -> m ()
equal f g = equalTo (f - g) zero
leq f g = leqTo (f - g) zero
geq = flip leq

{-# SPECIALIZE equal' :: (Ord v, Group c, AbelianAdditive c) => String -> LinFunc v c -> LinFunc v c -> LPM v c (),
        (Ord v, Group c, Monad m, AbelianAdditive c) => String -> LinFunc v c -> LinFunc v c -> LPT v c m () #-}
{-# SPECIALIZE geq' :: (Ord v, Group c, AbelianAdditive c) => String -> LinFunc v c -> LinFunc v c -> LPM v c (),
        (Ord v, Group c, Monad m, AbelianAdditive c) => String -> LinFunc v c -> LinFunc v c -> LPT v c m () #-}
{-# SPECIALIZE leq' :: (Ord v, Group c, AbelianAdditive c) => String -> LinFunc v c -> LinFunc v c -> LPM v c (),
        (Ord v, Group c, Monad m, AbelianAdditive c) => String -> LinFunc v c -> LinFunc v c -> LPT v c m () #-}
-- | Specifies the relationship between two functions in the variables, with a label on the constraint.
equal', leq', geq' :: (Ord v, Group c, MonadState (LP v c) m, AbelianAdditive c)
                   => String -> LinFunc v c -> LinFunc v c -> m ()
...
{-# SPECIALIZE addObjective :: (Ord v, Group c, AbelianAdditive c) => LinFunc v c -> LPM v c (),
        (Ord v, Group c, Monad m, AbelianAdditive c) => LinFunc v c -> LPT v c m () #-}
-- | Adds this function to the objective function.
addObjective :: (Ord v, Group c, MonadState (LP v c) m, AbelianAdditive c)
             => LinFunc v c -> m ()
